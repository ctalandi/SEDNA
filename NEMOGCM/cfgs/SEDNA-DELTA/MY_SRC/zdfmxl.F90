...
!CT SEDNA {
      INTEGER , DIMENSION(jpi,jpj) ::   zsedimld, zsednmln
      REAL(wp), DIMENSION(jpi,jpj) ::   zsedhmld, zsedhmlp
!CT SEDNA }
      !!----------------------------------------------------------------------
      !
      IF( kt == nit000 ) THEN
...
!CT Perfrom a different online diagnostic
      ...
      ! w-level of the mixing and mixed layers
      zsednmln(:,:)  = 2               ! Initialization to the number of w ocean point
      zsedhmlp(:,:)  = 0._wp               ! here hmlp used as a dummy variable, integrating vertically N^2
      DO jk = 2, jpkm1
         DO jj = 1, jpj                ! Mixed layer level: w-level 
            DO ji = 1, jpi
               ikt = mbkt(ji,jj)
               zsedhmlp(ji,jj) = zsedhmlp(ji,jj) + MAX( rn2b(ji,jj,jk) , 0._wp ) * e3w_n(ji,jj,jk)
      ...
      !
      ! w-level of the turbocline and mixing layer (iom_use)
      zsedimld(:,:) = mbkt(:,:) + 1        ! Initialization to the number of w ocean point
      DO jk = jpkm1, 2, -1         ! from the bottom to 2 
         DO jj = 1, jpj
            DO ji = 1, jpi
      ...
      DO jj = 1, jpj
         DO ji = 1, jpi
            iiki = zsedimld(ji,jj)
            iikn = zsednmln(ji,jj)
            zsedhmld (ji,jj) = gdepw_n(ji,jj,iiki  ) * ssmask(ji,jj)    ! Turbocline depth 
            zsedhmlp (ji,jj) = gdepw_n(ji,jj,iikn  ) * ssmask(ji,jj)    ! Mixed layer depth
         END DO
      END DO
!CT for SEDNA }

      IF( .NOT.l_offline ) THEN
         IF( iom_use("samldr10_1") ) THEN
            CALL iom_put( "samldr10_1", zsedhmlp )            ! mixed layer depth
         END IF
         IF( iom_use("samldkz5") ) THEN
