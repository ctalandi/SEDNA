MODULE zdfiwm
   ...
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:) ::   ecri_iwm   ! bottom-intensified dissipation at critical slopes (W/m2)
   ...
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:) ::   hcri_iwm   ! decay scale for critical slope dissipation (m)
...
CONTAINS
   ...
   SUBROUTINE zdf_iwm( kt, p_avm, p_avt, p_avs )
      ...
      !!                 1. Bottom-intensified dissipation at critical slopes, expressed
      ...
      !!                     zemx_iwm(z) = ( ebot_iwm / rau0 ) * ( 1 + hbot_iwm/H ) 
      !!                                   / ( 1 + (H-z)/hbot_iwm )^2                                
      ...
      !!              frequency (N^2).
      !!                     zemx_iwm(z) = ( ensq_iwm / rau0 ) * rn2(z)
      !!                                   / SUM( rn2(z) * e3w(z) )
      ...
      !!                     zemx_iwm(z) = ( esho_iwm / rau0 ) * sqrt(rn2(z))
      ...
      REAL(wp), DIMENSION(jpi,jpj)     ::   zhcri       ! Inverse of hcri_iwm
      ...
      zhcri(:,:) = 1._wp / hcri_iwm(:,:)  ! Compute inverse of hcri_iwm once for all
      DO jj = 1, jpj                ! part independent of the level
         DO ji = 1, jpi
            zhdep(ji,jj) = gdepw_0(ji,jj,mbkt(ji,jj)+1)       ! depth of the ocean
            zfact(ji,jj) = rau0 * (  1._wp - EXP( -zhdep(ji,jj) * zhcri(ji,jj) )  )
      ...
      DO jk = 2, jpkm1              ! complete with the level-dependent part
         DO jj = 1, jpj             
            DO ji = 1, jpi
               ...
               ELSE
                  zemx_iwm(ji,jj,jk) = zfact(ji,jj) * (  EXP( ( gde3w_n(ji,jj,jk  ) - zhdep(ji,jj) ) * zhcri(ji,jj) )     &
                       &                               - EXP( ( gde3w_n(ji,jj,jk-1) - zhdep(ji,jj) ) * zhcri(ji,jj) ) )   &
      ...
      DO jj = 1, jpj                ! part independent of the level
         DO ji = 1, jpi
            IF( zhdep(ji,jj) /= 0._wp )  zfact(ji,jj) = ebot_iwm(ji,jj) * (  1._wp +  hbot_iwm(ji,jj) / zhdep(ji,jj)  ) / rau0
         END DO
      END DO

      DO jk = 2, jpkm1              ! complete with the level-dependent part
         zemx_iwm(:,:,jk) = zemx_iwm(:,:,jk) +                                                                                  & 
            &               zfact(:,:) * (   1._wp / ( 1._wp + ( zhdep(:,:) - gde3w_n(:,:,jk  ) ) / hbot_iwm(:,:) )                      &
            &                             -  1._wp / ( 1._wp + ( zhdep(:,:) - gde3w_n(:,:,jk-1) ) / hbot_iwm(:,:) )   ) * wmask(:,:,jk)  &
      ...
      DO jk = 2, jpkm1              ! part independent of the level
         zfact(:,:) = zfact(:,:) + e3w_n(:,:,jk) * MAX( 0._wp, rn2(:,:,jk) )
      END DO

      DO jj= 1, jpj
         DO ji = 1, jpi
            IF( zfact(ji,jj) /= 0._wp )   zfact(ji,jj) = ensq_iwm(ji,jj) / ( rau0 * zfact(ji,jj) )
         END DO
      END DO

      DO jk = 2, jpkm1              ! complete with the level-dependent part
         zemx_iwm(:,:,jk) = zemx_iwm(:,:,jk) + zfact(:,:) * MAX( 0._wp, rn2(:,:,jk) )
      END DO
      ...
      zfact(:,:) = 0._wp
      DO jk = 2, jpkm1              ! part independent of the level
         zfact(:,:) = zfact(:,:) + e3w_n(:,:,jk) * SQRT(  MAX( 0._wp, rn2(:,:,jk) )  )
      END DO

      DO jj = 1, jpj
         DO ji = 1, jpi
             IF( zfact(ji,jj) /= 0._wp )   zfact(ji,jj) = esho_iwm(ji,jj) / ( rau0 * zfact(ji,jj) )
         END DO
      END DO

      DO jk = 2, jpkm1              ! complete with the level-dependent part
         zemx_iwm(:,:,jk) = zemx_iwm(:,:,jk) + zfact(:,:) * SQRT(  MAX( 0._wp, rn2(:,:,jk) )  )
      END DO
      !
      
      ! Calculate turbulence intensity parameter Reb
      DO jk = 2, jpkm1
         zReb(:,:,jk) = zemx_iwm(:,:,jk) / MAX( 1.e-20_wp, rnu * rn2(:,:,jk) )
      END DO
      !
      ! Define internal wave-induced diffusivity
      DO jk = 2, jpkm1
         zav_wave(:,:,jk) = zReb(:,:,jk) * r1_6 * rnu  ! This corresponds to a constant mixing efficiency of 1/6
      END DO
      !
   ...
   SUBROUTINE zdf_iwm_init
      ...
      !!              bottom-intensified dissipation at critical slopes (mixing_power_cri.nc)
      ...
      !!              decay scale for critical slope dissipation (decay_scale_cri.nc)
      !!
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namzdf_iwm in configuration namelist' )
      ...
      CALL iom_open('mixing_power_bot',inum)       ! energy flux for dissipation above abyssal hills [W/m2]
      CALL iom_get  (inum, jpdom_data, 'mixing_power_bot', ebot_iwm, 1 ) 
      CALL iom_close(inum)
      !
      CALL iom_open('mixing_power_cri',inum)       ! energy flux for dissipation at critical slopes [W/m2]
      CALL iom_get  (inum, jpdom_data, 'mixing_power_cri', ecri_iwm, 1 )
      CALL iom_close(inum)
      !
      CALL iom_open('mixing_power_nsq',inum)       ! energy flux for dissipation scaling with N^2 [W/m2]
      CALL iom_get  (inum, jpdom_data, 'mixing_power_nsq', ensq_iwm, 1 )
      CALL iom_close(inum)
       !
      CALL iom_open('mixing_power_sho',inum)       ! energy flux for dissipation due to shoaling [W/m2]
      CALL iom_get  (inum, jpdom_data, 'mixing_power_sho', esho_iwm, 1 )
      CALL iom_close(inum)
      !
      CALL iom_open('decay_scale_bot',inum)        ! spatially variable decay scale for abyssal hill dissipation [m]
      CALL iom_get  (inum, jpdom_data, 'decay_scale_bot', hbot_iwm, 1 )
      CALL iom_close(inum)
      !
      CALL iom_open('decay_scale_cri',inum)        ! spatially variable decay scale for critical-slope dissipation [m]
      CALL iom_get  (inum, jpdom_data, 'decay_scale_cri', hcri_iwm, 1 )
      CALL iom_close(inum)

      ebot_iwm(:,:) = ebot_iwm(:,:) * ssmask(:,:)
      ecri_iwm(:,:) = ecri_iwm(:,:) * ssmask(:,:)
      ensq_iwm(:,:) = ensq_iwm(:,:) * ssmask(:,:)
      esho_iwm(:,:) = esho_iwm(:,:) * ssmask(:,:)
      ...
      IF(lwp) THEN
         ...
         WRITE(numout,*) '      Dissipation at critical slopes:         ', zcri * 1.e-12_wp, 'TW'
